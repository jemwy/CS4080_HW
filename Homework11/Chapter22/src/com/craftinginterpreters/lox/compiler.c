  ...
  if (arg != -1) {
    if (canAssign && match(TOKEN_EQUAL)) {
      if (!current->locals[arg].isMutable) {
        error("Cannot assign to invar variable.");
      }
...
static void invarDeclaration() {
  uint8_t global = parseVariable("Expect variable name.", false);

  consume(TOKEN_EQUAL, "Expect initializer for invar declaration.");
  expression();

  consume(TOKEN_SEMICOLON, "Expect ';' after invar declaration.");
  defineVariable(global);
}
