...
static void whileStatement() {
  int enclosingLoopStart = currentLoopStart;
  int enclosingLoopScopeDepth = currentLoopScopeDepth;

  currentLoopStart = currentChunk()->count;
  currentLoopScopeDepth = current->scopeDepth;
  ...
  currentLoopStart = enclosingLoopStart;
  currentLoopScopeDepth = enclosingLoopScopeDepth;
}
...
static void continueStatement() {
  ...
  for (int i = current->localCount - 1;
       i >= 0 && current->locals[i].depth > currentLoopScopeDepth;
       i--) {
    emitByte(OP_POP);
  }
...
static void unlessStatement() {
  ...
  expression();
  consume(TOKEN_RIGHT_PAREN, "Expect ')' after unless condition.");
