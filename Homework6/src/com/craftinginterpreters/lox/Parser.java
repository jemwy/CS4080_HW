...
class Parser {
  ...
  private Stmt declaration() {
    try {
      ...
      if (match(FUN)) {
        ...
        return function("function", name, false, false);
      }
  ...
  private Stmt classDeclaration() {
    ...
    while (!check(RIGHT_BRACE) && !isAtEnd()) {
      ...
      methods.add(function("method", methodName, isStatic, isGetter));
    }
  ...
  private Stmt.Function function(String kind, Token name,
    ...
    boolean isSetter = 
      !isGetter && 
      parameters.size() == 1 && 
      !name.lexeme.equals("init");

    consume(LEFT_BRACE, "Expect '{' before " + kind + " body.");
    List<Stmt> body = block();

    return new Stmt.Function(name, parameters, body, 
      isStatic, isGetter, isSetter);
  }
  ...
  private Expr primary() {
    ...
    // Error productions for binary operators with no left-hand operand.
